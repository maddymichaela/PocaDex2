...
export function PhotocardCard({
  ...
  return (
    <motion.div
      ...
      className={`glass-card rounded-[28px] p-2.5 sm:p-3 shadow-md flex flex-col gap-3 relative overflow-hidden group cursor-pointer border-2 transition-all hover:shadow-xl hover:shadow-primary/5 ${selectMode && isSelected ? 'border-primary ring-4 ring-primary/10' : 'border-white/50'
        } ${isWishlist ? 'opacity-90 grayscale-[0.2]' : ''}`}
    >
      {/* Selection UI */}
      {selectMode && (
        <div className={`absolute top-4 left-4 z-20 w-6 h-6 rounded-xl border-2 flex items-center justify-center transition-all ${isSelected ? 'bg-primary border-primary text-white shadow-lg shadow-primary/20 scale-110' : 'bg-white/80 border-white'
          }`}>
          {isSelected && (
      ...
      >
        ...
        {!photocard.imageUrl && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <span className="text-center px-4 font-black italic text-primary/60 text-base">{photocard.member}</span>
          </div>
        )}
...
export function PhotocardGrid({
  ...
  return (
    <div
      className={`grid gap-3 sm:gap-6 animate-in fade-in duration-500 fill-mode-both ${layout === 'auto-fit'
