    ...
    return (
      ...
      >
        <motion.div
          ...
          className="bg-card rounded-[40px] p-8 md:p-10 max-w-sm w-full text-center space-y-8 shadow-2xl relative border-8 border-destructive/10"
        >
          <div className="w-20 h-20 bg-destructive/10 text-destructive rounded-3xl flex items-center justify-center mx-auto shadow-inner border-2 border-card/50 animate-bounce">
            <Trash size={32} />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button
              ...
              className="flex-1 py-4 bg-muted text-muted-foreground rounded-[20px] font-semibold text-xs uppercase tracking-widest border-2 border-border transition-all hover:bg-card hover:text-foreground"
            >
              Wait!
            </button>
            <button
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-card w-full h-full md:h-auto md:max-h-[90vh] md:max-w-4xl md:rounded-[48px] overflow-hidden shadow-2xl relative flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-center px-6 md:px-10 py-5 md:py-6 border-b border-border bg-card sticky top-0 z-30 font-sans">
          ...
          <button 
            onClick={onClose}
            className="p-2 md:p-3 bg-muted hover:bg-card border-2 border-border hover:border-primary/20 rounded-xl md:rounded-2xl transition-all text-foreground/20 hover:text-primary shadow-sm hover:shadow-md"
          >
            <X className="w-5 h-5 md:w-6 md:h-6" />
          </button>
        </div>

        <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar bg-card">
          <form id="photocard-form" onSubmit={handleSubmit} className="p-6 md:p-10 space-y-8 md:space-y-10 pb-24">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full px-4 md:px-6 py-3 md:py-4 bg-muted/50 border-border border-2 rounded-xl md:rounded-[24px] text-xs md:text-sm font-bold focus:ring-4 focus:ring-primary/10 focus:bg-card focus:border-primary/20 outline-none transition-all placeholder:text-foreground/10"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 md:gap-6">
                  ...
                  <div className="sm:col-span-1 space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full px-4 md:px-6 py-3 md:py-4 bg-muted/50 border-border border-2 rounded-xl md:rounded-[24px] text-xs md:text-sm font-black text-center focus:ring-4 focus:ring-primary/10 focus:bg-card focus:border-primary/20 outline-none transition-all appearance-none"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3">
                  ...
                  <div 
                    ...
                    className={`flex-1 min-h-[280px] md:min-h-[320px] bg-muted rounded-[32px] md:rounded-[40px] border-4 border-dashed flex flex-col items-center justify-center transition-all overflow-hidden relative group ${
                      previewUrl ? 'border-transparent shadow-inner' : 'border-border hover:border-primary/30 cursor-pointer'
                    }`}
                  >
                    {previewUrl ? (
                      <>
                       ...
                       <div className="absolute top-4 right-4 flex flex-col gap-3">
                         <button
                           ...
                           className="bg-card/90 backdrop-blur shadow-xl text-primary p-2 md:p-3 rounded-xl md:rounded-2xl hover:scale-110 active:scale-95 transition-all border border-card/50"
                           title="Edit Image"
                         >
                         ...
                         <button
                           ...
                           className="bg-card/90 backdrop-blur shadow-xl text-red-500 p-2 md:p-3 rounded-xl md:rounded-2xl hover:scale-110 active:scale-95 transition-all border border-card/50"
                           title="Remove Image"
                         >
                      ...
                      <>
                        <div className="relative z-10 flex flex-col items-center gap-4">
                          <div className="w-12 h-12 md:w-16 md:h-16 bg-card rounded-xl md:rounded-2xl flex items-center justify-center shadow-xl shadow-primary/5 transition-all group-hover:scale-110 group-hover:rotate-6 border border-border">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10 border-t border-border pt-8 md:pt-10">
              <div className="space-y-4">
                ...
                <div className="flex bg-muted p-1.5 md:p-2 rounded-2xl md:rounded-[24px] gap-1.5 md:gap-2">
                  {statusOptions.map((opt) => (
                    <button
                      ...
                      className={`flex-1 py-2.5 md:py-3 rounded-xl md:rounded-[18px] text-[9px] font-black transition-all uppercase tracking-tighter ${
                        status === opt.value ? `bg-card shadow-xl shadow-black/5 ${opt.color}` : 'text-foreground/30 hover:text-foreground hover:bg-card/50'
                      }`}
                    >
                      {opt.label}
                    </button>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-2 lg:grid-cols-5 gap-1.5 md:gap-2 bg-muted p-1.5 md:p-2 rounded-2xl md:rounded-[24px]">
                  {(['mint', 'near_mint', 'good', 'fair', 'poor'] as Condition[]).map((c) => (
                    <button
                      ...
                      className={`py-2.5 md:py-3 rounded-xl md:rounded-[18px] text-[9px] font-black transition-all uppercase tracking-tighter ${
                        condition === c ? 'bg-card shadow-xl shadow-black/5 text-primary' : 'text-foreground/30 hover:text-foreground hover:bg-card/50'
              ...
              <div className="space-y-1.5">
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-3 w-full px-4 md:px-6 py-3 md:py-4 rounded-xl md:rounded-[24px] border-2 transition-all font-black text-[9px] md:text-[10px] uppercase tracking-widest ${
                    isDuplicate ? 'bg-purple-500 border-purple-400 text-white shadow-xl shadow-purple-500/20 scale-[1.01]' : 'bg-muted/50 border-border text-foreground/20 hover:text-foreground/30'
                  } disabled:opacity-30`}
                >
                  <Copy size={16} />
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full px-4 md:px-6 py-4 md:py-6 bg-muted/50 border-border border-2 rounded-2xl md:rounded-[32px] text-xs md:text-sm font-bold focus:ring-4 focus:ring-primary/10 focus:bg-card focus:border-primary/20 outline-none transition-all h-28 md:h-36 resize-none placeholder:text-foreground/10 custom-scrollbar"
        ...
        <div className="shrink-0 flex gap-3 md:gap-4 p-4 md:px-10 md:pb-6 md:pt-4 bg-card border-t border-border z-30 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)]">
          {isEditing && (
            <button
              type="button"
              onClick={() => setShowConfirmDelete(true)}
              className="h-12 md:h-14 w-12 md:w-14 bg-destructive/10 text-destructive rounded-xl md:rounded-2xl font-black flex items-center justify-center hover:bg-red-500 hover:text-white transition-all border-card border-2 md:border-4 shadow-sm group shrink-0"
            >
          ...
          <button
            type="submit"
            form="photocard-form"
            className="flex-1 h-12 md:h-14 bg-primary text-white rounded-xl md:rounded-2xl font-black uppercase tracking-widest text-xs md:text-sm shadow-lg shadow-primary/30 hover:scale-[1.01] active:scale-[0.98] transition-all flex items-center justify-center gap-2 md:gap-3 group border-card/10 border-2 md:border-4"
          >
