...
export default function ImageEditor({ image, onSave, onCancel, aspectRatio = 1 / 1.5 }: ImageEditorProps) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-5xl bg-card rounded-[32px] md:rounded-[48px] overflow-hidden shadow-2xl flex flex-col h-full max-h-[96vh] md:max-h-[90vh] border-4 md:border-[12px] border-card">
        {/* Header */}
        <div className="px-5 md:px-8 py-3 md:py-5 flex justify-between items-center bg-card border-b border-border shrink-0">
           <div className="flex items-center gap-3">
            <div className="w-8 h-8 md:w-12 md:h-12 bg-primary/10 rounded-xl md:rounded-2xl flex items-center justify-center text-primary shadow-inner border border-card">
          ...
          <button 
            onClick={onCancel} 
            className="p-2 md:p-3 bg-muted hover:bg-card border-2 border-border hover:border-primary/20 rounded-xl md:rounded-2xl transition-all shadow-sm hover:shadow-md group"
          >
            <X className="text-foreground/30 group-hover:text-primary w-5 h-5 md:w-6 md:h-6" />
          </button>
        </div>

        <div className="flex-1 relative bg-muted flex items-center justify-center overflow-hidden min-h-[300px] md:min-h-[400px]">
        ...
        <div className="bg-card px-5 md:px-10 py-4 md:py-6 border-t border-border flex-1 overflow-y-auto">
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1 space-y-4">
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                   ...
                   <input
                    ...
                    className="w-full h-1.5 md:h-2 bg-muted rounded-full appearance-none cursor-pointer accent-primary"
                  />
              ...
              <div className="flex items-center gap-3">
                <button 
                  onClick={rotate}
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-muted border-2 border-border rounded-xl text-[9px] font-semibold uppercase tracking-widest text-muted-foreground hover:text-primary hover:bg-card transition-all shadow-sm"
                >
                  <RotateCw size={14} />
                  Rotate
                </button>
                <button 
                  onClick={resetAll}
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-muted border-2 border-border rounded-xl text-[9px] font-semibold uppercase tracking-widest text-foreground/30 hover:text-red-500 hover:bg-red-50 transition-all shadow-sm"
                >
                  <RotateCcw size={14} />
                  Reset
                </button>
        ...
        <div className="bg-card px-5 md:px-10 pb-6 md:pb-8 pt-4 border-t border-border shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)] shrink-0 space-y-6">
          {/* Adjustments Row */}
            ...
            {[
              ...
              <div key={adj.id} className="space-y-2">
                ...
                <input
                  ...
                  className="w-full h-1.5 bg-muted rounded-full appearance-none cursor-pointer accent-primary border border-card"
                />
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-3 md:gap-4">
            <button
              onClick={onCancel}
              className="flex-1 py-3.5 md:py-4 border-2 md:border-4 border-border bg-card text-muted-foreground rounded-xl md:rounded-[24px] font-semibold uppercase tracking-widest text-[9px] md:text-xs hover:bg-muted hover:text-foreground transition-all shrink-0"
            >
              Cancel
            </button>
            <button
              onClick={handleSave}
              className="flex-[2] py-3.5 md:py-4 bg-primary text-white rounded-xl md:rounded-[24px] font-semibold uppercase tracking-widest text-[9px] md:text-xs flex items-center justify-center gap-2 md:gap-3 shadow-lg shadow-primary/30 hover:scale-[1.01] active:scale-[0.98] transition-all border-card/20 border-2 md:border-4 shrink-0"
            >
